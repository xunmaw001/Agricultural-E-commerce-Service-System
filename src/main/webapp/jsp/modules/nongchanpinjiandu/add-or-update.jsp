...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑农产品监督</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>农产品监督管理</span></li>
				<li class="breadcrumb-item-two"><span>编辑农产品监督</span></li>
			</ol>
		</div>
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item">
					<label>商家账号</label>
					<input name="shangjiazhanghao" id="shangjiazhanghao" name="shangjiazhanghao" class="form-control" placeholder="请输入商家账号">
				</div>
				<div class="form-item">
					<label>店铺名称</label>
				...
				<div class="form-item">
					<label>温度</label>
					<input name="wendu" id="wendu" name="wendu" class="form-control" placeholder="请输入温度">
				</div>
				<div class="form-item">
					<label>湿度</label>
					<input name="shidu" id="shidu" name="shidu" class="form-control" placeholder="请输入湿度">
				</div>
				<div class="form-item">
					<label>状态</label>
					<select id="zhuangtaiSelect" name="zhuangtai" class="form-control">
							<option value="">请选择状态</option>
							<option class="zhuangtaiOption" value="正常">
								正常
							</option>
							<option class="zhuangtaiOption" value="良好">
								良好
							</option>
							<option class="zhuangtaiOption" value="较差">
								较差
							</option>
					</select>	
				</div>	
				<div class="form-item-file">
					<label>生长环境</label>
					<div class="upload btn-file">选择文件<input name="file" type="file" class="form-control-file" id="shengzhanghuanjingupload"></div>
					<div id="shengzhanghuanjingFileName" class="text"></div>
					<input name="shengzhanghuanjing" id="shengzhanghuanjing-input" type="hidden">
				</div>
				...
				<div class="form-item-file" >
					...
					<div class="upload btn-file">
						插入图片
						<input id="wuliuzhuangkuangupload" name="file" type="file">
					</div>
					<script id="wuliuzhuangkuangEditor" type="text/plain"></script>
					<script type="text/javascript">
						//建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
					   //相见文档配置属于你自己的编译器
					 var wuliuzhuangkuangUe = UE.getEditor('wuliuzhuangkuangEditor', {
						toolbars: [
							[
								'undo', //撤销
								'bold', //加粗
					...
					<input type="hidden" name="wuliuzhuangkuang" id="wuliuzhuangkuang-input">
				</div>
				<div class="form-btn">
	...
	<script>
		...
		var tableName = "nongchanpinjiandu";
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "nongchanpinjiandu/info/" + id,
		...
		function upload(){
			...
			$('#shengzhanghuanjingupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {				
					document.getElementById('shengzhanghuanjing-input').setAttribute('value',"upload/"+data.result.file);
					if(document.getElementById('shengzhanghuanjingFileName') != null){
						document.getElementById('shengzhanghuanjingFileName').innerHTML = "上传成功!";
					}				
					$("#shengzhanghuanjingImg").attr("src",baseUrl+"upload/"+data.result.file);												
				}
			});
			$('#wuliuzhuangkuangupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('wuliuzhuangkuangEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		function submit() {
			...
			if(validform() ==true && compare() == true){
				...
				if(crossrefid && crossuserid) {
					...
					http("nongchanpinjiandu/page","GET",{
						page:1,
						limit:10,
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("nongchanpinjiandu/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.wuliuzhuangkuang != null && ruleForm.wuliuzhuangkuang != 'null' && ruleForm.wuliuzhuangkuang != ''){
				var wuliuzhuangkuangMes = '' + ruleForm.wuliuzhuangkuang;
				var wuliuzhuangkuangUeditor = UE.getEditor('wuliuzhuangkuangEditor');
				wuliuzhuangkuangUeditor.ready(function() {
					wuliuzhuangkuangUeditor.setContent(wuliuzhuangkuangMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('wuliuzhuangkuangEditor').hasContents()){
				$('#wuliuzhuangkuang-input').attr('value',UE.getEditor('wuliuzhuangkuangEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("nongchanpinjiandu/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculationSE(colName){
			//单列求和接口
			http("nongchanpinjiandu"+colName,"GET",{
			...
			function fill(){
				...
				http(accountTableName+'/session','GET',{},(res)=>{
					if(res.code == 0){
						...
						if(res.data !=null && res.data.dianpumingcheng != null && res.data.dianpumingcheng != ''){
		...
		function dataBind(){
			...
			$("#shengzhanghuanjing-input").val(ruleForm.shengzhanghuanjing);
			$("#shengzhanghuanjingFileName").html(ruleForm.shengzhanghuanjing);
			$("#wuliuzhuangkuang").val(ruleForm.wuliuzhuangkuang);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.wendu != null && res.data.wendu != ''){

						$("#wendu").val(res.data.wendu);
						$("#wendu").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.shengzhanghuanjing != null && res.data.shengzhanghuanjing != ''){

						$("#shengzhanghuanjing-input").val(res.data.shengzhanghuanjing);
						$('#shengzhanghuanjingFileName').val(res.data.shengzhanghuanjing)
					}
					if(res.data != null && res.data != '' && res.data.wuliuzhuangkuang != null && res.data.wuliuzhuangkuang != ''){

						$("#wuliuzhuangkuang").val(res.data.wuliuzhuangkuang);
                        var wuliuzhuangkuangMes = '' + res.data.wuliuzhuangkuang;
                        var wuliuzhuangkuangUeditor = UE.getEditor('wuliuzhuangkuangEditor');
                        wuliuzhuangkuangUeditor.ready(function() {
                            wuliuzhuangkuangUeditor.setContent(wuliuzhuangkuangMes);
                        });
