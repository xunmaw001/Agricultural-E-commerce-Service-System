...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑助农信息</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>助农信息管理</span></li>
				<li class="breadcrumb-item-two"><span>编辑助农信息</span></li>
			</ol>
		</div>
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item">
					<label>店铺名称</label>
					<input id="dianpumingcheng" name="dianpumingcheng" class="form-control"  value="" readonly>
				</div>
				<div class="form-item">
					<label>提醒标题</label>
					<input name="tixingbiaoti" id="tixingbiaoti" name="tixingbiaoti" class="form-control" placeholder="请输入提醒标题">
				</div>
				...
				<div class="form-item">
					<label>提醒时间</label>
					<input id="tixingshijian-input" class="date" name="tixingshijian" size="20" type="text" readonly placeholder="请选择提醒时间">
				</div>
				...
				<div class="form-item-file" >
					...
					<div class="upload btn-file">
						插入图片
						<input id="tixingneirongupload" name="file" type="file">
					</div>
					<script id="tixingneirongEditor" type="text/plain"></script>
					...
					<input type="hidden" name="tixingneirong" id="tixingneirong-input">
				</div>
				<div class="form-btn">
	...
	<script>
		...
		function shangjiazhanghaoSelect2(){
			http("option/shangjia/shangjiazhanghao","GET",{
				tableName: "shangjia",
				...
				if(res.code == 0){
					...
					for(var i=0;i<options.length;i++){
						...
						//修改 回显
						if(ruleForm.shangjiazhanghao != null && ruleForm.shangjiazhanghao != '' && options[i] == ruleForm.shangjiazhanghao){
							shangjiazhanghaoOption.setAttribute("selected","selected");
						}
		...
		function shangjiazhanghaoSelect2Change(){
			...
			http("follow/shangjia/shangjiazhanghao","GET",{
				tableName: "shangjia",
				columnName: "shangjiazhanghao",
				columnValue: colVal
			},(res)=>{
				if(res.code == 0){
					if(res.data != null && res.data.dianpumingcheng != null){
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "zhunongxinxi/info/" + id,
		...
		function upload(){
			...
			$('#tixingneirongupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('tixingneirongEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("zhunongxinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("zhunongxinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('tixingneirongEditor').hasContents()){
				$('#tixingneirong-input').attr('value',UE.getEditor('tixingneirongEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("zhunongxinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dateTimePick(){
			...
			$("#tixingshijian-input").val(getCurDateTime());
			$("#tixingshijian-input").datetimepicker({ 
				autoclose: true,
		...
		function dataBind(){
			...
			$("#tixingbiaoti").val(ruleForm.tixingbiaoti);	
            if(ruleForm.tupian){
			...
			$("#tixingneirong").val(ruleForm.tixingneirong);	
		}		
		//图片显示
		function showImg(){
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.tixingbiaoti != null && res.data.tixingbiaoti != ''){

						$("#tixingbiaoti").val(res.data.tixingbiaoti);
						$("#tixingbiaoti").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.tixingneirong != null && res.data.tixingneirong != ''){
						...
						$("#tixingneirong").val(res.data.tixingneirong);
                        var tixingneirongMes = '' + res.data.tixingneirong;
                        var tixingneirongUeditor = UE.getEditor('tixingneirongEditor');
                        tixingneirongUeditor.ready(function() {
                            tixingneirongUeditor.setContent(tixingneirongMes);
                        });
